$(document).ready(function () {
	...
	function layerPopup() {
		...
		$('.popup-wrap .cancle').on('click', function () {
			$(this).closest('.popup-wrap').hide();
			$('body').css('overflow', 'auto');
			$this.focus();
		});
	...
	$(".btn_mypage").on("click", function(e){
		e.preventDefault();
		$(this).toggleClass("on");
		$(".mypage > ul").stop().slideToggle("fast");

		if(lang_state){
			$(this).attr("title", "마이페이지 메뉴 보기");
		}else{
			$(this).attr("title", "마이페이지 메뉴 닫기");
		}
		lang_state=!lang_state;
	});
	...
	function resevent() {
		...
		var resdepth01 = $('.m_bottom .m_info .info_dept01 > li');
		if(resdepth01.hasClass('menuover')){
			resdepth01.parents('.m_bottom').addClass('resdepthwrap');
		} else {
			resdepth01.parents('.m_bottom').removeClass('resdepthwrap');
		}
	}
	resevent()
	...
	//서브 탭 드롭박스
	function mbtnDropNav() {
		var mbtndropBtn = $('.mbtn-drop-tit');
		mbtndropBtn.on('click',function(){
			/*$(this).siblings('ul').slideToggle();*/
			$(this).toggleClass('on');
...
//서브 탭 드롭박스
	function mbtntabInpage() {
		var mbtntabBtn = $('.m_bottom.resdepthwrap .m_info .info_dept01 > li > a');
		var firTxt = $('.m_bottom.resdepthwrap .m_info .info_dept01 > li:first-child > a').text();
		$(".m_bottom.resdepthwrap .m_info .info_dept01.on").css({height:$(".m_bottom.resdepthwrap .m_info .info_dept01.on").outerHeight()+140+"px"});
		$('.mbtn-drop-tit').text(firTxt);
		mbtntabBtn.on('click',function(e){
			e.preventDefault();
			var thisTxt = $(this).text();
			$(this).parent().toggleClass('on');
			$(this).parent().siblings().removeClass('on');
			$(this).parents('.info_dept01').toggleClass('on');
			$(this).closest('.m_bottom').find('.mbtn-drop-tit').text(thisTxt);
			$(this).closest('.m_bottom').find('.mbtn-drop-tit').removeClass('on');			
		});
	}
	mbtntabInpage()
