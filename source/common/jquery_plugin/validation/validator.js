...
/*		            for(var i=0; i<str.length; i++) {
        ...
        "validate-is-imagevideo" : function(v) {
        	if(v) {
        		var ext = "";
	        	if(v.indexOf(".") >= 0) {
	        		ext = v.substring(v.lastIndexOf(".")+1);
	        		ext = ext.toLowerCase();
	        		if(ext.length != 3 || !(ext == "gif" || ext == "jpg" || ext == "jpeg" || ext == "png" || ext == "bmp" || ext == "mp4" || ext == "webm" || ext == "avi" || ext == "wmv" || ext == "mov"))
	        			return "파일은 이미지,동영상 파일만 등록을 하실수 있습니다.";
	        	} else {
	        		return "파일의 확장자가 없습니다.";
	        	}
        	}
